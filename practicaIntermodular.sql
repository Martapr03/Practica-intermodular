DROP DATABASE IF EXISTS practicaIntermodular;
CREATE DATABASE practicaIntermodular;
USE practicaIntermodular;

-- Crear tabla llamada Estrella
...
-- Creación del disparador trigger para actualizar el número de satélites en la tabla Planeta
CREATE TRIGGER actualiza_num_satelites
AFTER INSERT ON Satelite
FOR EACH ROW
BEGIN
  DECLARE num_satelites INT;
  -- Contar el número de satélites que tiene cada planeta
  SELECT COUNT(*) INTO num_satelites FROM Satelite WHERE nombre_planeta = NEW.nombre_planeta;
  -- Actualizar el número de satélites en la tabla Planeta
  UPDATE Planeta
  SET num_satelites = num_satelites
  WHERE nombre = NEW.nombre_planeta;
END;
//
DELIMITER ;

-- Insertar columnas en la tabla Estrella
...
-- Insertar columnas en la tabla Planeta
...
-- Insertar columnas en la tabla Satelite
